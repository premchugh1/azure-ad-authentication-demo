# 🔐 Azure AD Authentication - Sequence Diagram

## Traditional Sequence Diagram (ASCII Format)

```
┌─────────┐          ┌─────────┐          ┌─────────┐          ┌─────────┐          ┌─────────┐
│  User   │          │ React   │          │ Azure   │          │ Azure   │          │  JWKS   │
│ Browser │          │   SPA   │          │   AD    │          │Functions│          │Endpoint │
│         │          │(MSAL.js)│          │  (IDP)  │          │   API   │          │         │
└────┬────┘          └────┬────┘          └────┬────┘          └────┬────┘          └────┬────┘
     │                    │                     │                     │                     │
     │                    │                     │                     │                     │
     │  1. Navigate       │                     │                     │                     │
     │─────────────────>  │                     │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 2. Initialize MSAL  │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 3. Check Cached     │                     │                     │
     │                    │    Tokens           │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │  4. Show UI        │                     │                     │                     │
     │<─────────────────  │                     │                     │                     │
     │  (Login Button)    │                     │                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │                     │                     │
     │  5. Click "Login"  │                     │                     │                     │
     │─────────────────>  │                     │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 6. Generate PKCE    │                     │                     │
     │                    │    code_verifier    │                     │                     │
     │                    │    code_challenge   │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 7. Build Auth URL   │                     │                     │
     │                    │    with PKCE        │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │  8. Redirect to Azure AD                 │                     │                     │
     │  (with code_challenge)                   │                     │                     │
     │─────────────────────────────────────────>│                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │                     │                     │
     │  9. Show Login Page                      │                     │                     │
     │<─────────────────────────────────────────│                     │                     │
     │                    │                     │                     │                     │
     │  10. Enter Credentials                   │                     │                     │
     │─────────────────────────────────────────>│                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │ 11. Authenticate    │                     │
     │                    │                     │     Verify MFA      │                     │
     │                    │                     │     Check Consent   │                     │
     │                    │                     │────────┐            │                     │
     │                    │                     │        │            │                     │
     │                    │                     │<───────┘            │                     │
     │                    │                     │                     │                     │
     │                    │                     │ 12. Generate Code   │                     │
     │                    │                     │────────┐            │                     │
     │                    │                     │        │            │                     │
     │                    │                     │<───────┘            │                     │
     │                    │                     │                     │                     │
     │  13. Redirect with Auth Code             │                     │                     │
     │      (code + state)                      │                     │                     │
     │<─────────────────────────────────────────│                     │                     │
     │                    │                     │                     │                     │
     │  14. Callback URL  │                     │                     │                     │
     │─────────────────>  │                     │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 15. Extract Code    │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 16. POST /token     │                     │                     │
     │                    │     (code +         │                     │                     │
     │                    │     code_verifier)  │                     │                     │
     │                    │────────────────────>│                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │ 17. Verify PKCE     │                     │
     │                    │                     │     SHA256(verifier)│                     │
     │                    │                     │     == challenge    │                     │
     │                    │                     │────────┐            │                     │
     │                    │                     │        │            │                     │
     │                    │                     │<───────┘            │                     │
     │                    │                     │                     │                     │
     │                    │                     │ 18. Generate Tokens │                     │
     │                    │                     │────────┐            │                     │
     │                    │                     │        │            │                     │
     │                    │                     │<───────┘            │                     │
     │                    │                     │                     │                     │
     │                    │ 19. Return Tokens   │                     │                     │
     │                    │     access_token    │                     │                     │
     │                    │     id_token        │                     │                     │
     │                    │     refresh_token   │                     │                     │
     │                    │<────────────────────│                     │                     │
     │                    │                     │                     │                     │
     │                    │ 20. Cache Tokens    │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 21. Parse ID Token  │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │  22. Show Logged In│                     │                     │                     │
     │      (User Name)   │                     │                     │                     │
     │<─────────────────  │                     │                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │                     │                     │
     │  23. Click "Call   │                     │                     │                     │
     │      Protected API"│                     │                     │                     │
     │─────────────────>  │                     │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 24. Get Token       │                     │                     │
     │                    │     (with auto      │                     │                     │
     │                    │     refresh check)  │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 25. GET /api/protected                    │                     │
     │                    │ Authorization: Bearer <token>             │                     │
     │                    │──────────────────────────────────────────>│                     │
     │                    │                     │                     │                     │
     │                    │                     │                     │ 26. Extract Token   │
     │                    │                     │                     │────────┐            │
     │                    │                     │                     │        │            │
     │                    │                     │                     │<───────┘            │
     │                    │                     │                     │                     │
     │                    │                     │                     │ 27. Decode Header   │
     │                    │                     │                     │     (get 'kid')     │
     │                    │                     │                     │────────┐            │
     │                    │                     │                     │        │            │
     │                    │                     │                     │<───────┘            │
     │                    │                     │                     │                     │
     │                    │                     │                     │ 28. GET JWKS Keys   │
     │                    │                     │                     │────────────────────>│
     │                    │                     │                     │                     │
     │                    │                     │                     │ 29. Return Public   │
     │                    │                     │                     │     Keys (RSA)      │
     │                    │                     │                     │<────────────────────│
     │                    │                     │                     │                     │
     │                    │                     │                     │ 30. Find Matching   │
     │                    │                     │                     │     Key (kid)       │
     │                    │                     │                     │────────┐            │
     │                    │                     │                     │        │            │
     │                    │                     │                     │<───────┘            │
     │                    │                     │                     │                     │
     │                    │                     │                     │ 31. Verify JWT      │
     │                    │                     │                     │     Signature       │
     │                    │                     │                     │     (RS256)         │
     │                    │                     │                     │────────┐            │
     │                    │                     │                     │        │            │
     │                    │                     │                     │<───────┘            │
     │                    │                     │                     │                     │
     │                    │                     │                     │ 32. Validate Claims │
     │                    │                     │                     │     - audience      │
     │                    │                     │                     │     - issuer        │
     │                    │                     │                     │     - expiration    │
     │                    │                     │                     │────────┐            │
     │                    │                     │                     │        │            │
     │                    │                     │                     │<───────┘            │
     │                    │                     │                     │                     │
     │                    │                     │                     │ 33. Extract User    │
     │                    │                     │                     │     Claims          │
     │                    │                     │                     │────────┐            │
     │                    │                     │                     │        │            │
     │                    │                     │                     │<───────┘            │
     │                    │                     │                     │                     │
     │                    │ 34. Return 200 OK   │                     │                     │
     │                    │     + User Data     │                     │                     │
     │                    │<──────────────────────────────────────────│                     │
     │                    │                     │                     │                     │
     │  35. Display       │                     │                     │                     │
     │      API Response  │                     │                     │                     │
     │<─────────────────  │                     │                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │                     │                     │
     │  [Time passes - Token approaching expiration]                  │                     │
     │                    │                     │                     │                     │
     │                    │ 36. Check Token Exp │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 37. acquireTokenSilent                    │                     │
     │                    │     (refresh_token) │                     │                     │
     │                    │────────────────────>│                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │ 38. Validate Refresh│                     │
     │                    │                     │     Token           │                     │
     │                    │                     │────────┐            │                     │
     │                    │                     │        │            │                     │
     │                    │                     │<───────┘            │                     │
     │                    │                     │                     │                     │
     │                    │ 39. New Access Token│                     │                     │
     │                    │<────────────────────│                     │                     │
     │                    │                     │                     │                     │
     │                    │ 40. Update Cache    │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │                     │                     │
     │  41. Click "Logout"│                     │                     │                     │
     │─────────────────>  │                     │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 42. Clear Cache     │                     │                     │
     │                    │────────┐            │                     │                     │
     │                    │        │            │                     │                     │
     │                    │<───────┘            │                     │                     │
     │                    │                     │                     │                     │
     │                    │ 43. Redirect to     │                     │                     │
     │                    │     /logout         │                     │                     │
     │                    │────────────────────>│                     │                     │
     │                    │                     │                     │                     │
     │                    │                     │ 44. Clear Session   │                     │
     │                    │                     │────────┐            │                     │
     │                    │                     │        │            │                     │
     │                    │                     │<───────┘            │                     │
     │                    │                     │                     │                     │
     │                    │ 45. Redirect Back   │                     │                     │
     │                    │<────────────────────│                     │                     │
     │                    │                     │                     │                     │
     │  46. Show Logged   │                     │                     │                     │
     │      Out State     │                     │                     │                     │
     │<─────────────────  │                     │                     │                     │
     │                    │                     │                     │                     │
┌────┴────┐          ┌────┴────┐          ┌────┴────┐          ┌────┴────┐          ┌────┴────┐
│  User   │          │ React   │          │ Azure   │          │ Azure   │          │  JWKS   │
│ Browser │          │   SPA   │          │   AD    │          │Functions│          │Endpoint │
│         │          │(MSAL.js)│          │  (IDP)  │          │   API   │          │         │
└─────────┘          └─────────┘          └─────────┘          └─────────┘          └─────────┘
```

---

## 📋 **Simplified Flow Summary**

```
User Browser              React SPA              Azure AD              Azure Functions
     │                         │                       │                       │
     │───── Navigate ─────────>│                       │                       │
     │                         │                       │                       │
     │<──── Show Login ────────│                       │                       │
     │                         │                       │                       │
     │───── Click Login ──────>│                       │                       │
     │                         │                       │                       │
     │                         │── Generate PKCE ──>   │                       │
     │                         │                       │                       │
     │═══════════ Redirect with code_challenge ═══════>│                       │
     │                         │                       │                       │
     │<════════ Show Login Page ══════════════════════│                       │
     │                         │                       │                       │
     │═══════ Enter Credentials ═════════════════════>│                       │
     │                         │                       │                       │
     │                         │                       │── Authenticate ──>    │
     │                         │                       │                       │
     │<════ Redirect with Auth Code ══════════════════│                       │
     │                         │                       │                       │
     │────────────────────────>│                       │                       │
     │                         │                       │                       │
     │                         │── POST /token ───────>│                       │
     │                         │   (code + verifier)   │                       │
     │                         │                       │                       │
     │                         │                       │── Verify PKCE ──>     │
     │                         │                       │                       │
     │                         │<─ Return Tokens ──────│                       │
     │                         │   (access, id, refresh)                       │
     │                         │                       │                       │
     │                         │── Cache Tokens ──>    │                       │
     │                         │                       │                       │
     │<─── Show Logged In ─────│                       │                       │
     │                         │                       │                       │
     │─── Call Protected API ─>│                       │                       │
     │                         │                       │                       │
     │                         │──── GET /api/protected (Bearer token) ───────>│
     │                         │                       │                       │
     │                         │                       │                       │── Verify JWT ──>
     │                         │                       │                       │
     │                         │                       │                       │<─ Valid ────────
     │                         │                       │                       │
     │                         │<───── 200 OK + User Data ─────────────────────│
     │                         │                       │                       │
     │<─── Display Response ───│                       │                       │
     │                         │                       │                       │
     │                         │                       │                       │
     │                    [Token Refresh Cycle]        │                       │
     │                         │                       │                       │
     │                         │── Silent Refresh ────>│                       │
     │                         │                       │                       │
     │                         │<─ New Access Token ───│                       │
     │                         │                       │                       │
     │                         │                       │                       │
     │─── Click Logout ───────>│                       │                       │
     │                         │                       │                       │
     │                         │── Clear Cache ──>     │                       │
     │                         │                       │                       │
     │                         │── /logout ───────────>│                       │
     │                         │                       │                       │
     │                         │<─ Redirect ───────────│                       │
     │                         │                       │                       │
     │<─── Show Logged Out ────│                       │                       │
     │                         │                       │                       │
```

---

## 🎯 **Key Message Flows**

### **1. Login Flow (Steps 1-22)**
```
User → SPA: Click Login
SPA → SPA: Generate PKCE (code_verifier, code_challenge)
SPA → Azure AD: Redirect with code_challenge
Azure AD → User: Show Login Page
User → Azure AD: Enter Credentials
Azure AD → Azure AD: Authenticate & Generate Auth Code
Azure AD → SPA: Redirect with Auth Code
SPA → Azure AD: POST /token (code + code_verifier)
Azure AD → Azure AD: Verify PKCE (SHA256(verifier) == challenge)
Azure AD → SPA: Return Tokens (access, id, refresh)
SPA → SPA: Cache Tokens
SPA → User: Show Logged In State
```

### **2. API Call Flow (Steps 23-35)**
```
User → SPA: Click "Call API"
SPA → SPA: Get Access Token from Cache
SPA → API: GET /protected (Authorization: Bearer <token>)
API → API: Extract & Decode Token
API → JWKS: Fetch Public Keys
JWKS → API: Return RSA Public Keys
API → API: Verify Signature (RS256)
API → API: Validate Claims (aud, iss, exp)
API → API: Extract User Info
API → SPA: 200 OK + User Data
SPA → User: Display Response
```

### **3. Token Refresh Flow (Steps 36-40)**
```
SPA → SPA: Check Token Expiration (< 5 min remaining)
SPA → Azure AD: POST /token (grant_type=refresh_token)
Azure AD → Azure AD: Validate Refresh Token
Azure AD → SPA: Return New Access Token
SPA → SPA: Update Token Cache
```

### **4. Logout Flow (Steps 41-46)**
```
User → SPA: Click Logout
SPA → SPA: Clear Token Cache
SPA → Azure AD: Redirect to /logout
Azure AD → Azure AD: Clear Session
Azure AD → SPA: Redirect Back to App
SPA → User: Show Logged Out State
```

---

## 📊 **Actor Roles**

| Actor | Role | Responsibilities |
|-------|------|------------------|
| **User Browser** | End user interface | Initiates actions, displays UI, stores cookies |
| **React SPA (MSAL.js)** | Authentication client | PKCE generation, token management, API calls |
| **Azure AD (IDP)** | Identity provider | User authentication, token issuance, session management |
| **Azure Functions API** | Resource server | JWT validation, protected resource access |
| **JWKS Endpoint** | Public key provider | RSA public keys for signature verification |

---

## 🔐 **Security Checkpoints**

```
Checkpoint 1: PKCE Generation (Step 6)
├─ Generate random code_verifier (128 chars)
└─ Compute code_challenge = SHA256(code_verifier)

Checkpoint 2: State Parameter (Step 7)
├─ Generate random state value
└─ Validate on callback (CSRF protection)

Checkpoint 3: PKCE Verification (Step 17)
├─ Azure AD: SHA256(code_verifier) == stored code_challenge
└─ Only proceed if match ✅

Checkpoint 4: JWT Signature (Step 31)
├─ Verify signature with Azure AD public key
└─ Cryptographic proof of authenticity

Checkpoint 5: Claims Validation (Step 32)
├─ audience: Must be API client_id
├─ issuer: Must be Azure AD tenant
└─ expiration: Must be > current time
```

---

## ⏱️ **Timing & Performance**

```
Login Flow: ~2-5 seconds
├─ User Authentication: 1-3 seconds
├─ Token Exchange: 500ms - 1 second
└─ Token Caching: <100ms

API Call: ~200-500ms (first call)
├─ Token Retrieval: <50ms (from cache)
├─ HTTP Request: 100-300ms
└─ JWT Validation: 50-150ms

API Call: ~100-300ms (subsequent)
├─ JWKS Cached: 0ms (24h TTL)
└─ Validation Faster: 10-50ms

Token Refresh: ~500ms - 1 second
├─ Silent (background): No user impact
└─ Automatic: Triggered before expiry
```

---

## 🎯 **Message Types Legend**

```
─────>  Synchronous Request/Action
<─────  Synchronous Response
═════>  Redirect/Navigation (Browser)
<═════  Redirect/Navigation Response
────┐   Self-processing/Internal operation
    │
<───┘
```

---

**Created:** November 13, 2025  
**For:** Azure AD Authentication Demo  
**Format:** Traditional Vertical Sequence Diagram  
**Author:** GitHub Copilot Assistant
