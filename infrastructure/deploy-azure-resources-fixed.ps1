...
$API_APP_REG = az ad app create --display-name $API_CLIENT_ID_NAME --query appId -o tsv

# Update API app registration with identifier URI
Write-Host "Configuring API app registration..." -ForegroundColor Yellow
az ad app update --id $API_APP_REG --identifier-uris "api://$API_APP_REG"
...
$configContent | Out-File -FilePath "infrastructure/deployment-config.json" -Encoding UTF8
